...
public class JavaColorChooserPanel extends AbstractColorChooserPanel
{
    public void updateChooser()
    {
        ...
        String name = Configuration.getInstance().findJavaName(col);
        if (name != null)
        {
    ...
    {
        Map<String,Integer> names = Configuration.getInstance().getJavaNames();
        ...
        boxNames.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent)
            {
                ...
                if (name != null)
                {
                    Color color = Configuration.getInstance().findJavaColor(name);
                    if (color != null)
                    {
        ...
        Map<Integer,String> colors = Configuration.getInstance().getJavaColors();
    ...
    private static class ColorListCellRenderer extends JPanel implements ListCellRenderer
    {
        ...
        public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean hasFocus)
        {
            ...
            {
                text = (String)value;
                col = Configuration.getInstance().findJavaColor(text);
                text = String.format(ColorInfo.isHexUppercase() ? "%s - #%06X" : "%s - #%06x", text, col.getRGB() & 0xffffff);
            }
            else
            {
                col = new Color((Integer)value);
                text = String.format(ColorInfo.isHexUppercase() ? "#%06X - %s" : "#%06x - %s", col.getRGB() & 0xffffff, Configuration.getInstance().findJavaName(col));
            }
